...
class Point(object):
    ...
    def move_to(self,x,y):
        ...
        self.x = x
        self.y = y
        self.d_start = self.get_distance_from(Point(self.x_original,self.y_original))
        self.moves = self.moves + 1
        self.d_total = self.d_total + self.get_distance_from(p_start)
    ...
    def closer_to(self,p2,p3):
        d_1 = self.get_distance_from(p2)
        d_2 = self.get_distance_from(p3)
        if d_1>d_2:
            return p3
        else:
            return p2
    ...
    def halfway_to(self,point2):
        avg_x = (self.x + point2.x)/2
        avg_y = (self.y + point2.y)/2
